...
class Sankaku:
    ...
    def get_info_from_id(self, id: int):
        #### Doujin api
        if (res := self.__session.get(f"https://capi-v2.sankakucomplex.com/pools/{id}?lang=en")).status_code != 200:
        ...
        #### Normal api
        if (res := self.__session.get(f"https://capi-v2.sankakucomplex.com/posts?lang=en&page=1&limit=1&tags=id_range:{id}")).status_code != 200:
    ...
    def _getPage(self, page: int = None):
        ...
        return json.loads(
            self.__session.get(API_URL + "posts/keyset", params=params).content
        )
    ...
    def download(self):
        ...
        for i in range(total):
            self.output(f"[Sankaku] D({i+1}/{total}): {posts[i][POST_ID]} | {posts[i][POST_SIZE] // 1000}kb")
